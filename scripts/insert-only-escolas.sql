-- Script SUPER SIMPLES para Supabase (em caso de erro com o script principal)
-- Execute APENAS se o script principal falhar

-- 1. Primeiro, vamos criar a constraint de forma manual
-- (Execute apenas se ainda não existir)
ALTER TABLE escolas ADD CONSTRAINT unique_escola_nome UNIQUE (nome);

-- 2. Se der erro de constraint já existir, execute apenas isto:
